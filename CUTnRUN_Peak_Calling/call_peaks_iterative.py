...
def main(argv):
	...
	smooth_ar = savgol_filter(np.array(nval_ar),9,1)
	...
	for j in profile.keys(): #Every chromosome
		...
		peaks, _ = find_peaks(smooth_ar, height=it_cutoff, distance=25)
		
		for p in peaks:
			peak_pos = pos_list[p]
			peak_val = temp_list[p]
			temp_str = j + "\t" + str(peak_pos-15) + "\t" + str(peak_pos+15) + "\t" + j + "_" + str(peak_pos) + "\t" + str(peak_val) + "\n"
			fh.write(temp_str)
