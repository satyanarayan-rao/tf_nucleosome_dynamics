...
dt_sub_ggplot$col_to_plot_med = dt_sub_ggplot$col_to_plot/median_val_for_dotted_line 
kk = ggplot(dt_sub_ggplot, 
     aes (x = assigned_cl_id, y = col_to_plot_med, fill = assigned_cl_id, alpha = 0.5)) + 
...
kk = kk + ylim (c(min(dt_sub_ggplot$col_to_plot_med), max(dt_sub_ggplot$col_to_plot_med) )) 
kk = kk + ylim(c(min(dt_sub_ggplot$col_to_plot_med),3))
...
if (identical(min_val, as.integer("-1000"))){
    ...
    kk = kk + geom_hline (yintercept  = 1, #yintercept=median_val_for_dotted_line, 
