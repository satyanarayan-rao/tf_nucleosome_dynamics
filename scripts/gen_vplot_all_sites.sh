...
nsites=`wc -l ${11} | awk '{print $1}'`
start_sites=`wc -l ${18} | awk '{print $1}'`
fraction=`echo "scale=3; ${nsites}/${start_sites}" | bc -l`
percent=`echo "${fraction}*100" | bc -l` 

cnt_th=`echo $1 | awk -F'_tfbs_th_count_' '{print $NF}' | awk -F'_' '{print $1}'`
fragment_th=`echo $1 | awk -F'_in_range_' '{print $NF}' | sed 's:.tsv$::g;s:_to_:, :g' | awk '{print "["$0"]"}'`
...
set title "${5} on ${6}\n(min $cnt_th frag. in $fragment_th; n = ${nsites} (${percent}%) )"   noenhanced
set xra [0:$xra]
...
convert ${13%.pdf}.tmp.png -background white -pointsize 75 label:"${5} on ${6}\n(min $cnt_th frag. in $fragment_th; n = ${nsites} (${percent}%))"  +swap -gravity Center -append ${13%.pdf}.png
...
convert ${17%.pdf}.tmp.png -background white -pointsize 75 label:"${5} on ${6}\n(min $cnt_th frag. in $fragment_th; n = ${nsites} (${percent}%))"  +swap -gravity Center -append ${17%.pdf}.png
