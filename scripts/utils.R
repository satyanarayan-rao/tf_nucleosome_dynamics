...
read_from_connection = function(con=NULL, 
	...
	return (NULL) # cat("Finished reading from connection!\n")
        ...
        df = read.csv(text=lines, sep=sep, header=FALSE)
        return(df)
    }
}
...
find_reads_center_in_tfbs = function(df=NULL, it=NULL){
    ...
    mid_point = as.integer((df$chromStart + df$chromStop)/2) 
    #df$`chromStart` = df$`chromStart` + mid_point
    df$`chromStart` = mid_point 
    df$`chromStop` = df$`chromStart` # just keeping one bp
